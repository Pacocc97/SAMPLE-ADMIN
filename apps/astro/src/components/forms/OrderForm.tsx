...
export default function OrderForm({ user }) {
  ...
  const [getCartProducts, setCartProducts] = createSignal([]);
  const [getShippingInfo, setShippingInfo] = createSignal<ShippingInfo>({});
  const [getBillingInfo, setBillingInfo] = createSignal<BillingInfo>({});
  const [getCardInfo, setCardInfo] = createSignal<CardInfo>({});
  const [disabledbutton, setDisabledButton] = createSignal(false);
  const [getNewShipping, setNewShipping] = createSignal(
    user.address.length > 0 ? false : true,
  );
  const [getNewBilling, setNewBilling] = createSignal(
    user.address.length > 0 ? false : true,
  );
  const [getSameAsShipping, setSameAsShipping] = createSignal(true);
  const [getSelectedShipping, setSelectedShipping] =
    createSignal<ShippingInfo | null>(null);
  const [getSelectedBilling, setSelectedBilling] =
    createSignal<BillingInfo | null>(getSelectedShipping());
  const [deviceSessionId, setDeviceSessionId] = createSignal("");
  const [tokenId, setTokenId] = createSignal("");
  const [getIsLoading, setIsLoading] = createSignal(false);

  const userDiscount = 1 - user?.role?.discount / 10000 || 0;
  createEffect(() => {
  ...
  createEffect(() => {
    ...
    const setupOpenPay = () => {
      window.OpenPay.setId(import.meta.env.PUBLIC_OPENPAY_MERCHANT_ID);
      window.OpenPay.setApiKey(import.meta.env.PUBLIC_OPENPAY_PUBLIC_API_KEY);
  ...
  const handlePayButtonClick = (event: MouseEvent) => {
    event.preventDefault();
    const shippingData = getNewShipping()
      ? getShippingInfo()
      : getSelectedShipping();
    const billingData = getSameAsShipping()
      ? shippingData
      : getNewBilling()
      ? getBillingInfo()
      : getSelectedBilling();
    if (!shippingData || !billingData) {
      if (!shippingData) {
        Toast.fire({
          title: "Elija una dirección de envío",
          icon: "error",
        });
        return;
      } else if (!billingData) {
        Toast.fire({
          title: "Elija una dirección de facturación",
          icon: "error",
        });
        return;
      }
  ...
  const successCallback = async (response: any) => {
    ...
    const { id, userId, finality, identifier, ...passShipping } =
      shippingData as ShippingInfo;
    const {
      id: ID,
      userId: USER,
      finality: FIN,
      identifier: IDENT,
      ...passBilling
    } = billingData as BillingInfo;

    const obj = {
      // cardData,
      shippingAddress: passShipping,
      billingAddress: passBilling,
    };
    await apiPublic.openpay.create
      .mutate({
        source_id: tokenId(),
        device_session_id: deviceSessionId(),
        userId: user && user.id,
        productArray: getCartProducts(),
        client_name: getCardInfo().holder_name,
        email: getEmail(),
      })
      .then(
        async (val) =>
          await apiPublic.order.create
            .mutate({
              openPayId: val.id,
              productArray: getCartProducts().map((p) => {
                return {
                  currentPrice: Math.round(p.price * userDiscount),
                  quantity: p.quantity,
                  id: p.id,
                };
              }),
  ...
  const handleChange = (e: InputEvent, type: string) => {
    ...
    if (type === "shipping") {
      setShippingInfo((values) => ({ ...values, [name]: value }));
    } else if (type === "billing") {
      setBillingInfo((values) => ({ ...values, [name]: value }));
    } else if (type === "card") {
      setCardInfo((values) => ({ ...values, [name]: value }));
    }
  };

  function cardType(value: string | undefined) {
    if (value) {
      switch (value) {
        case "37":
          return { name: "amex", number: 15, cvc: 4 };
        case "34":
          return { name: "amex", number: 15, cvc: 4 };
        default:
          return { name: null, number: 16, cvc: 3 };
      }
    } else {
      return { name: null, number: 16, cvc: 3 };
    }
              ...
              <div class="col-span-3 sm:col-span-4">
                ...
                <div class="relative mt-1">
                  <input
                    ...
                    type="text"
                    maxlength={
                      cardType(getCardInfo()?.["card_number"]?.substring(0, 2))
                        .number
                    }
                    autocomplete="off"
                  ...
                  <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                    {getCardInfo()["card_number"] &&
                      (getCardInfo()["card_number"]?.substring(0, 2) ===
                      "34" ? (
                        <svg
                          class="-ml-1 mr-2 h-3 w-10"
                          viewBox="0 0 256 64"
                          fill="none"
                      ...
                      ) : getCardInfo()["card_number"]?.substring(0, 1) ===
                        "5" ? (
                        <svg
                          class="-ml-1 mr-2 h-3 w-10"
                          viewBox="0 0 660 203"
...
-->*/}
              {!getNewShipping() &&
                user?.address?.map((address: ShippingInfo) => (
                  ...
                  <label class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none">
                    ...
                    <span class="flex flex-1">
                      <span class="flex flex-col">
                        ...
                        >
                          {`${address.street} ${address.streetNumber}, ${address.neighborhood}, ${address.municipality}, ${address.state}, ${address.postalCode}.`}
                        </span>
                        {/* <span
                    ...
                    {/*<!-- Not Checked: "hidden" -->*/}
                    {getSelectedShipping()?.id === address.id && (
                      <svg
            ...
            {user.address.length > 0 && (
              <button
                type="button"
                onclick={() => setNewShipping((val) => !val)}
                class="mt-5 rounded-md bg-gray-600 px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-gray-400"
              >
                {!getNewShipping()
                  ? "Dirección nueva"
                  : "Ver direcciones guaradadas"}
              </button>
            )}
            {getNewShipping() && (
              <div class="mt-6 grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-6">
                <div class="sm:col-span-6">
                  ...
                  <div class="mt-1">
                    <input
                      onInput={(e) => handleChange(e, "shipping")}
                      type="text"
                      name="street"
                      class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    />
          ...
          <section aria-labelledby="shipping-heading" class="mt-10">
            ...
            <div class="mt-6 flex items-center">
              <input
                ...
                type="checkbox"
                onchange={() => setSameAsShipping((val) => !val)}
                checked={getSameAsShipping()}
              ...
              <div class="ml-2">
                ...
                >
                  Usar misma dirección del envío
                </label>
              </div>
            </div>
            <div class="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4">
        ...
        -->*/}
              {!getSameAsShipping() &&
                !getNewBilling() &&
                user?.address?.map((address: BillingInfo) => (
                  // eslint-disable-next-line react/jsx-key
                  <label class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none">
                    <input
                      type="radio"
                      name="delivery-method"
                      value="Standard"
                      onchange={() => setSelectedBilling(address)}
                      class="sr-only"
                      aria-labelledby="delivery-method-0-label"
                      aria-describedby="delivery-method-0-description-0 delivery-method-0-description-1"
                    />
                    ...
                    {getSelectedBilling()?.id === address.id && (
                      <svg
                        class="h-5 w-5 text-indigo-600"
                        viewBox="0 0 20 20"
                        fill="currentColor"
            ...
            {!getSameAsShipping() && user.address.length > 0 && (
              <button
                type="button"
                onclick={() => setNewBilling((val) => !val)}
                class="mt-5 rounded-md bg-gray-600 px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-gray-400"
              >
                {!getNewBilling()
                  ? "Dirección nueva"
            ...
            {!getSameAsShipping() && getNewBilling() && (
              <div class="mt-6 grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-6">
                <div class="sm:col-span-6">
                  <label
                    for="company"
                  ...
                  <div class="mt-1">
                    <input
                      onInput={(e) => handleChange(e, "billing")}
                      type="text"
                      name="street"
                      class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    />
