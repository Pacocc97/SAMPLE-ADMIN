...
export function InsertImageUriDialogBody({
  ...
  return (
    <>
      <TextInput
        label="URL Imagen"
        placeholder="e.j. https://source.unsplash.com/random"
        onChange={setSrc}
        value={src}
  ...
  const loadImage = (files: FileList | null) => {
    ...
    reader.onload = async function () {
      if (typeof reader.result === "string") {
        ...
        if(imageResponse)
        setSrc(
          `https://d26xfdx1w8q2y3.cloudfront.net/${imageResponse?.path}/${imageResponse?.original}`,
        );
...
function onDragStart(event: DragEvent): boolean {
  ...
  dataTransfer.setDragImage(img as HTMLImageElement, 0, 0);
