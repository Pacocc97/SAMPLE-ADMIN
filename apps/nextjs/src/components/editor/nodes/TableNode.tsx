...
export function extractRowsFromHTML(tableElem: HTMLTableElement): Rows {
  ...
  for (let y = 0; y < rowElems.length; y++) {
    ...
    const cellElems = (rowElem as HTMLTableRowElement).querySelectorAll(
      "td,th",
    );
...
export function exportTableCellsToHTML(
  ...
  for (
    ...
    x < (rect != null ? rect.endX + 1 : (firstRow as Row).cells.length);
    x++
  ) {
  ...
  for (
    ...
    for (
      ...
      const cellElem = document.createElement(
        (cell as Cell).type === "header" ? "th" : "td",
      );
      cellElem.innerHTML = cellHTMLCache.get((cell as Cell).json) || "";
...
export class TableNode extends DecoratorNode<JSX.Element> {
  ...
  mergeRows(startX: number, startY: number, mergeRows: Rows): void {
    ...
    for (let y = startY; y < endY; y++) {
      ...
      const mergeCells = (mergeRow as Row).cells;
      ...
      for (let x = startX; x < endX; x++) {
        ...
        const cellClone = {
          ...cell,
          json: (mergeCell as Cell).json,
          type: (mergeCell as Cell).type,
        };
        (cellsClone[x] as any) = cellClone;
      }
      (rows[y] as any) = rowClone;
    }
  }
  ...
  updateCellType(x: number, y: number, type: "header" | "normal"): void {
    ...
    const cell = cells?.[x];
    const cellsClone = Array.from(cells as any);
  ...
  insertColumnAt(x: number): void {
    ...
    for (let y = 0; y < rows.length; y++) {
      ...
      cellsClone.splice(x, 0, createCell(type as any));
      (rows[y] as any) = rowClone;
    }
  }
  ...
  addColumns(count: number): void {
    ...
    for (let y = 0; y < rows.length; y++) {
      ...
      for (let x = 0; x < count; x++) {
        cellsClone.push(createCell(type as any));
      }
      (rows[y] as any) = rowClone;
    }
  }
  ...
  insertRowAt(y: number): void {
    ...
    const cellCount = (prevRow as Row).cells.length;
    const row = createRow();
    for (let x = 0; x < cellCount; x++) {
      const cell = createCell((prevRow?.cells[x] as any).type );
      row.cells.push(cell);
    }
    rows.splice(y, 0, row);
  }
  ...
  updateColumnWidth(x: number, width: number): void {
    ...
    for (let y = 0; y < rows.length; y++) {
      ...
      const cellsClone = Array.from(cells);
      const rowClone = { ...row, cells: cellsClone };
      (cellsClone[x] as any).width = width;
      (rows[y] as any) = rowClone;
    }
  }
