...
function getSelectedIDs(
  ...
  for (let x = startX; x <= endX; x++) {
    for (let y = startY; y <= endY; y++) {
      // Perform safety checks before accessing properties
      const row = rows[y];
      if (row) {
...
function extractCellsFromRows(
  ...
  for (let y = startY; y <= endY; y++) {
    const row = rows[y];
    if (row) { // Add a safety check to ensure 'row' is defined
      const newRow = createRow();
      for (let x = startX; x <= endX; x++) {
        const cellClone = { ...(row.cells[x]) };
        cellClone.id = createUID();
        newRow.cells.push(cellClone as any);
      }
      newRows.push(newRow);
    }
  }
...
function TableActionMenu({
  ...
  return (
    ...
    >
      ...
      >
        <span className="text">Insertar fila arriba</span>
      </button>
      <button
        className="item"
        onClick={() => {
      ...
      >
        <span className="text">Insertar fila debajo</span>
      </button>
      <hr />
      <button
        className="item"
      ...
      >
        <span className="text">Insertar columna izquierda</span>
      </button>
      <button
        className="item"
        onClick={() => {
      ...
      >
        <span className="text">Insertar columna derecha</span>
      </button>
      <hr />
      {(rows[0] as any).cells.length !== 1 && (
        <button
          className="item"
          onClick={() => {
            updateTableNode((tableNode) => {
...
export default function TableComponent({
  ...
  const rows = useMemo(() => {
    ...
    _rows.sort((a, b) => {
      ...
      const aContent = cellTextContentCache.get((aCells[x] as any).json) || "";
      const bContent = cellTextContentCache.get((bCells[x] as any).json) || "";
    ...
    _rows.unshift(rawRows[0] as any);
    return _rows;
  ...
  const modifySelectedCells = useCallback(
    (x: number, y: number, extend: boolean) => {
      const id = ((rows[y] as any).cells[x] ).id;
  ...
  useEffect(() => {
    ...
    const pasteContent = async (event: ClipboardEvent) => {
      let clipboardData: null | DataTransfer | ClipboardItem | any =
      ...
      editor.registerCommand<KeyboardEvent>(
        ...
        (event, targetEditor) => {
          ...
          if (
            ...
            const firstCellID = ((rows[0] as any).cells[0] ).id;
        ...
        (event) => {
          ...
          if (
            ...
            if (x === 0 && isBackward) {
              if (y !== 0) {
                nextY = y - 1;
                nextX = (rows[nextY] as any).cells.length - 1;
              }
            } else if (x === (rows[y] as any).cells.length - 1 && !isBackward) {
              if (y !== rows.length - 1) {
                nextY = y + 1;
                nextX = 0;
              }
      ...
      editor.registerCommand<KeyboardEvent>(
        ...
        (event, targetEditor) => {
          ...
          if (!isEditing && selection === null) {
            ...
            if (cellID !== null) {
              const [x, y] = cellCoordMap.get(cellID) as [number, number];
              if (x !== (rows[y] as any).cells.length - 1) {
                modifySelectedCells(x + 1, y, extend);
                return true;
              }
            }
